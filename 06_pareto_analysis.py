...
OBJECTIVES = {
    "montague_min_flow_mgd": True,       # Maximize minimum flow (higher = better for ecology)
    "nyc_min_storage_pct": True,         # Maximize minimum storage (higher = better for supply)
    "pct_time_drought_watch": False,     # Minimize time in drought watch
...
def plot_objectives_parallel_coordinates(metrics_df, pareto_mask, objective_names,
    ...
    objectives = metrics_df[objective_names].values

    # Get baseline values if provided
    baseline_values = None
    if baseline_metrics is not None:
        baseline_values = np.array([baseline_metrics.get(name, np.nan)
                                    for name in objective_names])

    # Normalize objectives to [0, 1] for visualization
    # Include baseline in bounds calculation so it fits on the plot
    obj_min = objectives.min(axis=0)
    obj_max = objectives.max(axis=0)

    if baseline_values is not None and not np.any(np.isnan(baseline_values)):
        # Expand bounds to include baseline
        obj_min = np.minimum(obj_min, baseline_values)
        obj_max = np.maximum(obj_max, baseline_values)

    obj_range = obj_max - obj_min + 1e-10

    objectives_norm = (objectives - obj_min) / obj_range

    # Flip minimization objectives so "up" is always better
    for i, maximize in enumerate(maximize_flags):
        if not maximize:
            objectives_norm[:, i] = 1 - objectives_norm[:, i]

    # Plot dominated solutions (gray, thin)
    for i in np.where(~pareto_mask)[0]:
        ax.plot(range(n_obj), objectives_norm[i],
                color='gray', alpha=0.1, linewidth=0.5)

    # Plot Pareto solutions (colored)
    pareto_objectives = objectives_norm[pareto_mask]

    # Color by average normalized objective (overall "goodness")
    avg_performance = pareto_objectives.mean(axis=1)
    colors = plt.cm.RdYlGn((avg_performance - avg_performance.min()) /
                           (avg_performance.max() - avg_performance.min() + 1e-10))

    for sample, color in zip(pareto_objectives, colors):
        ax.plot(range(n_obj), sample, color=color, alpha=0.8, linewidth=2)

    # Plot baseline if provided
    if baseline_values is not None and not np.any(np.isnan(baseline_values)):
        # Normalize baseline using same bounds
        baseline_norm = (baseline_values - obj_min) / obj_range
        # Flip minimization objectives
        for i, maximize in enumerate(maximize_flags):
            if not maximize:
                baseline_norm[i] = 1 - baseline_norm[i]

        ax.plot(range(n_obj), baseline_norm, color='blue', linewidth=3,
                marker='o', markersize=8, label='Baseline', zorder=15)
        ax.legend(loc='lower right', fontsize=9)

    # Create axis labels with direction indicator
    labels = []
    ...
    ax.set_ylabel('Normalized Value (↑ = better)', fontsize=10)
    ax.set_title('Pareto Front in Objective Space\n(gray=dominated, colored=Pareto, blue=baseline)', fontsize=11)
    ...
    sm = plt.cm.ScalarMappable(cmap='RdYlGn',
                               norm=plt.Normalize(avg_performance.min(),
                                                  avg_performance.max()))
    sm.set_array([])
    cbar = plt.colorbar(sm, ax=ax, shrink=0.8)
    cbar.set_label('Avg. Normalized Performance', fontsize=9)
